...
static HTTPRouterCookieStorage globalCookiesStoragePolicy = HTTPRouterCookiesShared;

@interface HTTPRequestFileUpload : NSObject
...
@interface HTTPRequestDataUpload : NSObject
...
@property (nonatomic, strong) NSMutableArray *dataToUpload;
@property (nonatomic, strong) NSURLRequest *request;
@property (nonatomic, strong) NSURL *HTTPBodyFileURL;
...
+ (instancetype) requestWithURL:(NSURL *)url {
    ...
    return [(HTTPRouter *)[self alloc] initWithURL:url];
}

+ (instancetype) requestWithURLRoute:(NSString *)route {
    NSURL * finalURL = [[NSURL alloc] initWithString:[NSString stringWithFormat:@"%@%@", kHTTPRootAddress, route]];
    return [self requestWithURL:finalURL];
}
...
- (instancetype) initWithURL:(NSURL *)address {
    if (self = [super init]) {
        ...
        self.encodePOSTDictionary   = true;
        self.encodeGETDictionary    = true;
        self.addCredentialsToURL    = false;
        self.timeoutSeconds         = kHTTPRouterDefaultTimeout;
        self.filesToUpload          = [NSMutableArray array];
        self.dataToUpload           = [NSMutableArray array];
        self.HTTPMethod             = @"GET";
        self.cookieStoragePolicyForInstance = HTTPRouterCookieStorageUndefined;
        self.ephemeralRequestCookies = [NSMutableArray array];
    }
    NSLog(@"initWithURL:(NSURL *)address: %@", address);
    return self;
}
...
- (void) setUsername:(NSString *)username password:(NSString *)password {
    NSURLCredential *c = [NSURLCredential credentialWithUser:username password:password persistence:NSURLCredentialPersistenceNone];
...
- (NSURLRequest *)prepareURLRequest {
    ...
    if([self.filesToUpload count] > 0 || [self.dataToUpload count] > 0) {
        ...
        for(HTTPRequestFileUpload *fileToUpload in self.filesToUpload) {
        ...
        for (HTTPRequestDataUpload *dataToUpload in self.dataToUpload) {
        NSData *multipartData = [[self class] multipartContentWithBoundary:boundary
...
- (void)addFileToUpload:(NSString *)path parameterName:(NSString *)parameterName {
    ...
    HTTPRequestFileUpload *fu = [HTTPRequestFileUpload fileUploadWithPath:path parameterName:parameterName];
    [self.filesToUpload addObject:fu];
}

- (void)addDataToUpload:(NSData *)data parameterName:(NSString *)param {
    HTTPRequestDataUpload *du = [HTTPRequestDataUpload dataUploadWithData:data parameterName:param mimeType:nil fileName:nil];
...
- (void)addDataToUpload:(NSData *)data parameterName:(NSString *)param mimeType:(NSString *)mimeType fileName:(NSString *)fileName {
    HTTPRequestDataUpload *du = [HTTPRequestDataUpload dataUploadWithData:data parameterName:param mimeType:mimeType fileName:fileName];
    [self.dataToUpload addObject:du];
}
...
- (NSString *)curlDescription {
    ...
    for(HTTPRequestFileUpload *f in _filesToUpload) {
...
- (NSString *)debugDescription {
    ...
    for(HTTPRequestFileUpload *f in self.filesToUpload) {
        [ms appendString:@"UPLOAD FILE\n"];
        [ms appendFormat:@"\t %@ = %@\n", f.parameterName, f.path];
    }
    
    for(HTTPRequestDataUpload *d in self.dataToUpload) {
...
- (void)startAsynchronous {
    ...
    if(_useUploadTaskInBackground) {
        ...
        if ([[NSURLSessionConfiguration class] respondsToSelector:
...
- (void)cancel {
    ...
    self.error = [NSError errorWithDomain:NSStringFromClass([self class]) code:kHTTPRouterCancelationError userInfo:@{NSLocalizedDescriptionKey: s}];
...
@implementation NSError (HTTPRouter)
...
- (BOOL)st_isCancellationError {
    return ([[self domain] isEqualToString:@"STHTTPRequest"] && [self code] == kHTTPRouterCancelationError);
}
@end
...
- (NSString *)st_stringByAppendingGETParameters:(NSDictionary *)parameters doApplyURLEncoding:(BOOL)doApplyURLEncoding {
    ...
    NSArray *sortedParameters = [HTTPRouter dictionariesSortedByKey:parameters];
...
@implementation HTTPRequestFileUpload
+ (instancetype)fileUploadWithPath:(NSString *)path parameterName:(NSString *)parameterName mimeType:(NSString *)mimeType {
    HTTPRequestFileUpload *fu = [[self alloc] init];
    fu.path = path;
...
@implementation HTTPRequestDataUpload
+ (instancetype)dataUploadWithData:(NSData *)data parameterName:(NSString *)parameterName mimeType:(NSString *)mimeType fileName:(NSString *)fileName {
    HTTPRequestDataUpload *du = [[self alloc] init];
    du.data = data;
